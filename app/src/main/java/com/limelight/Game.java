...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    // Up to 10 touches are supported
    private final TouchContext[] touchContextMap = new TouchContext[MAX_TOUCH_POINTS];
    private final SparseIntArray pointerIdToSlotMap = new SparseIntArray();
    private final boolean[] slotsBusy = new boolean[MAX_TOUCH_POINTS];
    ...
    private int assignSlotForPointer(int pointerId) {
        int slot = pointerIdToSlotMap.get(pointerId, -1);
        if (slot != -1) return slot;

        for (int i = 0; i < MAX_TOUCH_POINTS; i++) {
            if (!slotsBusy[i]) {
                slotsBusy[i] = true;
                pointerIdToSlotMap.put(pointerId, i);
                return i;
            }
        }
        return -1;
    ...
    private void releaseSlotForPointer(int pointerId) {
        int slot = pointerIdToSlotMap.get(pointerId, -1);
        if (slot != -1) {
            slotsBusy[slot] = false;
            pointerIdToSlotMap.delete(pointerId);
        }
    }

    private TouchContext getTouchContextForPointer(int pointerId)
    {
        int slot = pointerIdToSlotMap.get(pointerId, -1);
        if (slot != -1) {
            return touchContextMap[slot];
        }
        return null;
    }
                 ...
                 eventSource == InputDevice.SOURCE_MOUSE_RELATIVE)
        {
                    ...
                    eventSource == 12290) // 12290 = Samsung DeX mode desktop mouse
            {
                ...
                if (inputCaptureProvider.eventHasRelativeMouseAxes(event)) {
                    ...
                    deltaX = (short) (deltaX * sensitivity);
                    deltaY = (short) (deltaY * sensitivity);
            ...
            else
            {
                ...
                case MotionEvent.ACTION_POINTER_DOWN:
                case MotionEvent.ACTION_DOWN:
                    int slot = assignSlotForPointer(pointerId);
                    if (slot == -1) {
                        return false;
                    }
                    TouchContext context = touchContextMap[slot];
                ...
                case MotionEvent.ACTION_POINTER_UP:
                case MotionEvent.ACTION_UP:
                    TouchContext upContext = getTouchContextForPointer(pointerId);
                    if (upContext == null) {
                        return false;
                    }
                    ...
                    if (event.getPointerCount() == 1 &&
                        ...
                        if (event.getEventTime() - threeFingerDownTime < THREE_FINGER_TAP_THRESHOLD) {
                            ...
                            toggleKeyboard();
                            releaseSlotForPointer(pointerId);
                            return true;
                        }
                    }

                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU && (event.getFlags() & MotionEvent.FLAG_CANCELED) != 0) {
                        upContext.cancelTouch();
                    }
                    else {
                        upContext.touchUpEvent(eventX, eventY, event.getEventTime());
                    }
                    ...
                    int upSlot = pointerIdToSlotMap.get(pointerId, -1);
                    releaseSlotForPointer(pointerId);

                    if (upSlot == 0 && event.getPointerCount() > 1 && !upContext.isCancelled()) {
                        // The original primary touch (slot 0) is gone.
                        // We handover the primary status to the next finger.
                        int nextIndex = (actionIndex == 0) ? 1 : 0;
                        int nextPointerId = event.getPointerId(nextIndex);

                        // If the next finger is already in a slot, release it
                        releaseSlotForPointer(nextPointerId);

                        // Reassign it to slot 0
                        pointerIdToSlotMap.put(nextPointerId, 0);
                        slotsBusy[0] = true;

                        // Inform Slot 0 context about the handover finger
                        touchContextMap[0].touchDownEvent(
                                (int)(event.getX(nextIndex) + xOffset),
                                (int)(event.getY(nextIndex) + yOffset),
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    for (int h = 0; h < event.getHistorySize(); h++) {
                        for (int i = 0; i < event.getPointerCount(); i++) {
                            ...
                            TouchContext ctx = getTouchContextForPointer(pId);
                            if (ctx != null) {
                                ctx.touchMoveEvent(
                                        (int)(event.getHistoricalX(i, h) + xOffset),
                                        (int)(event.getHistoricalY(i, h) + yOffset),
                    ...
                    for (int i = 0; i < event.getPointerCount(); i++) {
                        ...
                        if (ctx != null) {
                            ctx.touchMoveEvent(
                                    (int)(event.getX(i) + xOffset),
                                    (int)(event.getY(i) + yOffset),
                                    event.getEventTime());
                        }
                    }
                    break;
                case MotionEvent.ACTION_CANCEL:
                    ...
                    for (int i = 0; i < MAX_TOUCH_POINTS; i++) {
                        if (slotsBusy[i]) {
                            touchContextMap[i].cancelTouch();
                            touchContextMap[i].setPointerCount(0);
                            slotsBusy[i] = false;
                        }
                    }
                    pointerIdToSlotMap.clear();
                    break;
                default:
                    return false;
                }
